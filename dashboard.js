...
ganttScrollLeft.addEventListener('dblclick', () => {
    ganttContainer.scrollTo({ left: 0, behavior: 'smooth' });
...
ganttScrollRight.addEventListener('dblclick', () => {
    ganttContainer.scrollTo({ left: ganttContainer.scrollWidth, behavior: 'smooth' });
...
function scrollToFirstTask() {
    const chart = charts.gantt;
    const tasks = chart && chart.currentTasks ? chart.currentTasks : [];
    if (tasks.length > 0) {
        const dates = tasks.map(t => parseDateLocal(t.start)).filter(d => !isNaN(d));
        if (dates.length > 0) {
            ganttContainer.scrollTo({ left: 0, behavior: 'smooth' });
        }
    }
